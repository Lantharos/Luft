...
pub(super) unsafe fn texture_program(
    ...
    let create_variant = |defines: &[&str]| -> Result<GlesTexProgramVariant, GlesError> {
        ...
        let debug_shader = src.replace(
            "//_DEFINES_",
            &defines.iter().chain(&[shaders::DEBUG_FLAGS]).fold(
                String::new(),
                |mut shader, define| {
        ...
        let debug_program =
            unsafe { link_program(gl, shaders::VERTEX_SHADER, debug_shader.as_ref())? };
        ...
        Ok(GlesTexProgramVariant {
            ...
            debug: GlesTexProgramInternal {
                ...
                uniform_tex: gl
                    .GetUniformLocation(debug_program, tex.as_ptr() as *const ffi::types::GLchar),
...
pub(super) unsafe fn solid_program(gl: &ffi::Gles2) -> Result<GlesSolidProgram, GlesError> {
    let program = link_program(
        gl,
        shaders::VERTEX_SHADER_SOLID,
        shaders::FRAGMENT_SHADER_SOLID,
    )?;
