...
impl super::PhysicalDevice {
    ...
    pub(super) unsafe fn from_phd(
        ...
        if let Some(info) = unsafe {
            PhdInfo::from_phd(instance.handle(), instance.api_version(), phd, &extensions)
        } {
            let span =
                info_span!(parent: &instance.0.span, "backend_vulkan_device", name = info.name);
            Ok(Some(Self {
                phd,
                info,
                extensions,
...
impl super::PhdInfo {
    ...
    unsafe fn from_phd(
        ...
        let api_version =
            Version::from_raw(u32::min(properties.api_version, instance_version.to_raw()));
        ...
        info.driver = info
            .properties_driver
            .map(DriverInfo::from_driver_properties);
