    ...
    fn damage_since(
        ...
        self.damage
            ...
            .map(|d| {
                d.into_iter()
                    .map(|d| {
                        d.to_logical(1, self.transform, &self.slot.size())
                            .to_physical(1)
                    })
                    .collect()
            })
    ...
    F: Framebuffer,
{
    ...
    pub fn damage_from_age<'d>(
        ...
        (
            Option<&'d Vec<Rectangle<i32, Physical>>>,
            RenderElementStates,
        ),
        OutputNoMode,
    >
        ...
        E: Element,
    {
        ...
        let mut elements: Vec<FrameResultDamageElement<'_, '_, E, B>> = Vec::with_capacity(
            usize::from(self.cursor_element.is_some()) + self.overlay_elements.len() + 1,
        );
        ...
        let primary_dmabuf = match &self.primary_element {
            PrimaryPlaneElement::Swapchain(PrimarySwapchainElement { slot, sync, .. }) => {
                let dmabuf = match &slot.buffer {
                    ScanoutBuffer::Swapchain(slot) => {
                        slot.export().map_err(BlitFrameResultError::Export)?
                    }
                    _ => unreachable!(),
                };
                let size = dmabuf.size();
                let geometry =
                    Rectangle::from_size(size.to_logical(1, Transform::Normal).to_physical(1));
...
impl<B: Buffer + std::fmt::Debug, F: Framebuffer + std::fmt::Debug, E: std::fmt::Debug>
    std::fmt::Debug for RenderFrameResult<'_, B, F, E>
{
