...
use crate::{
    ...
    input::{
        ...
        tablet::{
            ...
            tool::{
                self, AxisFrame, ButtonEvent, DownEvent, MotionEvent, TabletToolGrab,
                TabletToolHandle, TabletToolInternal, TabletToolRc, TabletToolTarget, UpEvent,
                WeakTabletToolHandle,
            },
        },
    },
...
impl<D: TabletSeatHandler + 'static> TabletToolHandle<D> {
    ...
    pub fn client_tools<'a>(
        &'a self,
        client: &Client,
    ) -> impl Iterator<Item = ZwpTabletToolV2> + 'a {
        ...
        F: Fn() -> Box<dyn TabletToolGrab<D>> + Send + 'static,
    {
        ...
        for seat in instances.iter() {
            ...
            if let Ok(client) = dh.get_client(seat.id()) {
                tool.arc.wp_tablet_tool.new_instance(
                    state,
                    &client,
                    dh,
                    &seat,
    ...
    fn proximity_in<D: TabletSeatHandler + 'static>(
        ...
        *self.last_proximity_in.lock().unwrap() = Some(serial);
        ...
        self.for_each_focused_tool(surface, |wp_tool| {
            ...
            let Some(wp_tablet) = tablet
                .arc
                .wp_tablet
                .focused_tablet_for_seat(surface, seat_id)
            else {
                return;
            };
...
impl Drop for WpTabletToolHandle {
    fn drop(&mut self) {
        ...
        for tool in guard.drain(..) {
            let Ok(wp_tool) = tool.upgrade() else {
                continue;
            };
    ...
    D: TabletSeatHandler + 'static,
{
    fn proximity_in(
        ...
        if let Some(tool) = tablet_seat.get_tool(tool_descriptor) {
            tool.arc
                .wp_tablet_tool
                .proximity_in::<D>(self, tablet, serial);
        }
    }
