...
impl EGLExtensions {
    fn from_display(egl_version: (i32, i32), display: *const c_void) -> Result<Self, Error> {
        ...
        let has_export_dmabuf = extensions
            .iter()
...
unsafe fn select_platform_display<N: EGLNativeDisplay + 'static>(
    ...
    for platform in native.supported_platforms() {
        ...
        info!(
            "Successfully selected EGL platform: {}",
...
impl EGLDisplay {
        ...
        N: EGLNativeDisplay + 'static,
    {
        ...
        let (dmabuf_import_formats, dmabuf_render_formats) =
            get_dmabuf_formats(&display.handle, &extensions)
                .map_err(Error::DisplayCreationError)?;
    ...
    fn get_extensions(
        ...
        if egl_version >= (1, 2) {
            let p = unsafe {
                CStr::from_ptr(
                    wrap_egl_call_ptr(|| {
                        ffi::egl::QueryString(display, ffi::egl::EXTENSIONS as i32)
                    })
                    .map_err(Error::InitFailed)?,
                )
            };
    ...
    pub fn choose_config(
        ...
        let mut config_ids: Vec<ffi::egl::types::EGLConfig> =
            Vec::with_capacity(num_configs as usize);
        wrap_egl_call_bool(|| unsafe {
            ffi::egl::ChooseConfig(
                **self.display,
                descriptor.as_ptr(),
    ...
    pub(super) unsafe fn get_pixel_format(
        &self,
        config_id: *const c_void,
    ...
    pub(super) fn supports_damage_impl(&self) -> DamageSupport {
            ...
            .has_extension("EGL_KHR_swap_buffers_with_damage")
        {
            ...
            .extensions
            .has_extension("EGL_EXT_swap_buffers_with_damage")
        {
            DamageSupport::EXT
        } else {
            DamageSupport::No
    ...
    pub fn create_dmabuf_from_image(
        ...
        dma.build()
            .ok_or(Error::DmabufExportFailed(EGLError::BadAlloc))
    }
    ...
    pub fn egl_buffer_contents(
        ...
        let format = match format {
            ...
            ffi::egl::TEXTURE_Y_UV_WL => {
                return Err(BufferAccessError::UnsupportedMultiPlanarFormat(
                    Format::Y_UV,
                ));
            }
