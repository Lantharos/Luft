...
pub use self::handlers::{
    RegionUserData, SubsurfaceCachedState, SubsurfaceUserData, SurfaceUserData,
};
...
use wayland_server::protocol::{
    wl_buffer, wl_callback, wl_output, wl_region, wl_surface::WlSurface,
};
...
where
    ...
    D: 'static,
{
    let (user_state_type_id, user_state_type) =
        surface.data::<SurfaceUserData>().unwrap().user_state_type;
    assert_eq!(
        std::any::TypeId::of::<D>(),
        user_state_type_id,
    ...
    fn region_attributes_add_subtract_add() {
        let region = RegionAttributes {
            rects: vec![
                ...
                (
                    RectangleKind::Add,
                    Rectangle::new((2, 2).into(), (2, 2).into()),
                ),
            ],
        };
