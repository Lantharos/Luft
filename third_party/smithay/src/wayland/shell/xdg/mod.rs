...
impl WmCapabilitySet {
    ...
    pub fn replace(
        &mut self,
        capabilities: impl IntoIterator<Item = xdg_toplevel::WmCapabilities>,
    ) {
...
impl ShellClient {
    ...
    pub fn send_ping(&self, serial: Serial) -> Result<(), PingError> {
        ...
        let user_data = self
            .kind
            .data::<self::handlers::XdgWmBaseUserData>()
            .unwrap();
...
impl ToplevelSurface {
    ...
    fn get_pending_state(
        &self,
        attributes: &mut XdgToplevelSurfaceRoleAttributes,
    ) -> Option<ToplevelState> {
    ...
    pub fn send_configure(&self) -> Serial {
        ...
        let (configure, decoration_mode_changed, bounds_changed, capabilities_changed) =
            compositor::with_states(&self.wl_surface, |states| {
                ...
                let bounds_changed =
                    !attributes.initial_configure_sent || (pending.bounds != current.bounds);

                // test if we should send a capabilities event, either because the
                // capabilities changed or we never sent one
                let capabilities_changed = !attributes.initial_configure_sent
                    || (pending.capabilities != current.capabilities);
    ...
    pub(crate) fn pre_commit_hook<D: XdgShellHandler + 'static>(
        ...
        if let Some((error, msg)) = error {
            ...
            if let Some(toplevel) = toplevels
                .iter()
...
impl PopupSurface {
    ...
    pub fn send_configure(&self) -> Result<Serial, PopupConfigureError> {
        ...
        compositor::with_states(&self.wl_surface, |states| {
            ...
            if attributes.initial_configure_sent
                && self.version() < xdg_popup::EVT_REPOSITIONED_SINCE
            {
        ...
        let res = compositor::with_states(surface, |states| {
            ...
            if grab.is_some() && had_buffer_before {
                ...
                if let Some(popup) = popups.iter().find(|handle| handle.wl_surface == *surface) {
                    popup.shell_surface.post_error(
