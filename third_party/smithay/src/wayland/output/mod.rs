...
impl Output {
    ...
    pub(crate) fn wl_change_current_state(
        ...
        for output in &inner.instances {
            ...
            let scale_changed =
                client_scale != data.last_client_scale.swap(client_scale, Ordering::AcqRel);
            ...
            if (new_scale.is_some() || scale_changed) && output.version() >= 2 {
                let scale = (inner.scale.integer_scale() as f64 / client_scale)
                    .max(1.)
                    .ceil() as i32;
                output.scale(scale);
            }
