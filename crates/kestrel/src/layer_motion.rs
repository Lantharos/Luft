...
use smithay::{
    ...
    desktop::{Space, WindowSurfaceType, layer_map_for_output},
    output::Output,
    reexports::wayland_server::protocol::wl_surface::WlSurface,
    utils::{Logical, Physical, Point, Rectangle, Scale, Size},
    wayland::{
        compositor::{get_role, with_states},
        shell::wlr_layer::LAYER_SURFACE_ROLE,
    },
};
...
impl LayerMotionState {
    ...
    pub fn observe_surface_commit(
        ...
        let output_size = output_logical_size(output);
        if !renderer_surface_mapped(surface) && !outside_output(geometry, output_size) {
            return;
        }
    ...
    pub fn offsets(
        ...
        for layer in layers {
            ...
            let mapped = renderer_surface_mapped(layer.wl_surface());
...
fn output_logical_size(output: &Output) -> Size<i32, Logical> {
    output
        .current_mode()
        .map(|mode| {
            output
...
fn renderer_surface_mapped(surface: &WlSurface) -> bool {
