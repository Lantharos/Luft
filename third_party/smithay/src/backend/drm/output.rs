...
use crate::{
    backend::{
        ...
        renderer::{
            Bind, Color32F, DebugFlags, Renderer, RendererSuper, Texture, element::RenderElement,
        },
    },
...
type CompositorList<A, F, U, G> =
    Arc<RwLock<HashMap<crtc::Handle, Mutex<DrmCompositor<A, F, U, G>>>>>;
                        ...
                        match compositor {
                            ...
                            Err(err) => {
                                ...
                                for compositor in self.compositor.values_mut() {
                                    ...
                                    render_elements
                                        .submit_composited_frame(&mut *compositor, renderer)?;
                                }
...
fn use_mode_internal<'a, A, F, U, G, R, E>(
    compositor_list: &mut RwLockWriteGuard<
        'a,
        HashMap<crtc::Handle, Mutex<DrmCompositor<A, F, U, G>>>,
    >,
    crtc: &crtc::Handle,
    mode: Mode,
    allocator: &A,
    ...
    R::Error: Send + Sync + 'static,
{
    ...
    if let Err(err @ FrameError::DrmError(DrmError::TestFailed(_))) = res.as_ref() {
        tracing::warn!(
            ?crtc,
            ?err,
            "failed to set mode, trying to lower bandwidth usage"
        );
    ...
    R::Error: Send + Sync + 'static,
{
        ...
        U: 'static,
    {
        ...
        if frame_result.needs_sync() {
            if let PrimaryPlaneElement::Swapchain(primary_swapchain_element) =
