...
use crate::utils::{
    Buffer as BufferCoords, IsAlive, Rectangle, Size, Transform, user_data::UserDataMap,
};
...
fn validate_dmabuf(
    ...
    let dma_constraints = constraints
        .dma
        .as_ref()
...
impl FrameInner {
    fn new(
        session_obj: ExtImageCopyCaptureSessionV1,
        constraints: Option<BufferConstraints>,
    ) -> Self {
        Self {
            constraints,
        ...
        match request {
            ...
            ext_image_copy_capture_manager_v1::Request::CreatePointerCursorSession {
                ...
                if let Some(constraints) =
                    state.cursor_capture_constraints(&capture_source, &pointer)
                {
                    session_ref.update_constraints(constraints);
                    state
                        .image_copy_capture_state()
        ...
        match request {
            ...
            ext_image_copy_capture_frame_v1::Request::Capture => {
                ...
                self.inner
                    .lock()
                    .unwrap()
                    .fail(resource, FailureReason::Unknown);
            }
