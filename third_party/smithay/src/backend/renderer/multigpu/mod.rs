...
use super::{
    ...
    ImportDma, ImportMem, Offscreen, Renderer, RendererSuper, Texture, TextureFilter,
    TextureMapping,
    sync::{self, SyncPoint},
};
...
where
    ...
    <<R::Device as ApiDevice>::Renderer as RendererSuper>::Error:
        Into<SwapBuffersError> + Send + Sync,
    <<T::Device as ApiDevice>::Renderer as RendererSuper>::Error:
        Into<SwapBuffersError> + Send + Sync,
{
    #[inline]
                ...
                if let Some(data) = states.data_map.get::<RendererSurfaceStateUserData>() {
                    ...
                    if data.textures.is_empty() {
                        ...
                        if let Some(buffer) = data.buffer.as_ref() {
                            ...
                            let buffer_damage =
                                data.damage().raw().take(1).flatten().cloned().fold(
        ...
        <<A::Device as ApiDevice>::Renderer as ExportMem>::TextureMapping: 'static,
    {
        match buffer_type(buffer) {
            Some(BufferType::Dma) => {
                ...
                let mut texture =
                    MultiTexture::from_surface(Some(surface), dmabuf.size(), dmabuf.format());

                if !self
                    .devices
                    .iter()
                    .any(|device| target_node == *device.node())
                {
                    return Err(Error::DeviceMissing);
                }
                ...
                let src_node =
                    import_on_src_node(dmabuf, Some(damage), &mut texture, first, None, devices)?;
                if src_node != target_node {
                    let target_id = self
                        .devices
                        .iter()
                        .find_map(|dev| {
                            (*dev.node() == target_node).then(|| dev.renderer().context_id())
                        })
                        .unwrap();
                    let src_id = self
                        .devices
                    ...
                    let mappings = {
                        let damage = damage
                            .iter()
                            .flat_map(|rect| {
                                rect.intersection(Rectangle::from_size(src_texture.size()))
                            })
                            .fold(
                                Vec::<Rectangle<i32, BufferCoords>>::new(),
                                |damage, mut rect| {
...
impl<'a, T: GraphicsApi>
    AsRef<<<T::Device as ApiDevice>::Renderer as RendererSuper>::Framebuffer<'a>>
...
impl<'a, T: GraphicsApi>
    AsMut<<<T::Device as ApiDevice>::Renderer as RendererSuper>::Framebuffer<'a>>
    for MultiFramebuffer<'a, T>
{
    fn as_mut(
        &mut self,
    ) -> &mut <<T::Device as ApiDevice>::Renderer as RendererSuper>::Framebuffer<'a> {
        match &mut self.0 {
            MultiFramebufferInternal::Render(fb) => fb,
            MultiFramebufferInternal::Target(fb) => fb,
        }
    ...
    <<T::Device as ApiDevice>::Renderer as RendererSuper>::Error: 'static,
{
    ...
    framebuffer: Option<
        AliasableBox<<<R::Device as ApiDevice>::Renderer as RendererSuper>::Framebuffer<'frame>>,
    >,
...
struct TargetFrameData<'target, 'frame, 'buffer, T: GraphicsApi> {
    ...
    framebuffer:
        &'frame mut <<T::Device as ApiDevice>::Renderer as RendererSuper>::Framebuffer<'buffer>,
        ...
        if let Some(target) = self.target.as_ref() {
            ...
            let renderer = unsafe {
                std::mem::transmute::<
                    ...
                    &<T::Device as ApiDevice>::Renderer,
                >(self.render.renderer())
            };
...
impl<'render, 'target, R: GraphicsApi, T: GraphicsApi> RendererSuper
...
impl<'render, 'target, R: GraphicsApi, T: GraphicsApi> Renderer
    for MultiRenderer<'render, 'target, R, T>
where
    R: 'static,
    R::Error: 'static,
        ...
        let frame = match &mut framebuffer.0 {
            MultiFramebufferInternal::Render(framebuffer) => {
                ...
                let framebuffer = unsafe {
                    std::mem::transmute::<
                        &mut <<T::Device as ApiDevice>::Renderer as RendererSuper>::Framebuffer<
                            'buffer,
                        >,
                        &mut <<R::Device as ApiDevice>::Renderer as RendererSuper>::Framebuffer<
                            'buffer,
                        >,
                    >(framebuffer)
                };
            ...
            MultiFramebufferInternal::Target(target_framebuffer) => {
                ...
                let mut render_framebuffer =
                    AliasableBox::from_unique(Box::new(render_framebuffer));
                ...
                let frame = unsafe {
                    std::mem::transmute::<
                        ...
                        <<R::Device as ApiDevice>::Renderer as RendererSuper>::Frame<
                            'frame,
                            'buffer,
                        >,
                    >(
    ...
    fn finish_internal(&mut self) -> Result<sync::SyncPoint, Error<R, T>> {
        if let Some(frame) = self.frame.take() {
            ...
            let mut damage = std::mem::take(&mut self.damage)
                ...
                .map(|rect| {
                    rect.to_logical(1)
                        .to_buffer(1, self.dst_transform, &damage_area)
                })
                .collect::<Vec<_>>();
            ...
            if let Some(target) = self.target.as_mut() {
                if let Some(texture) = target.texture.as_ref() {
                    ...
                    let damage = damage
                        .iter()
                        .map(|rect| {
                            rect.to_logical(1, Transform::Normal, &buffer_size)
                                .to_physical(1)
                        })
                        .collect::<Vec<_>>();
                ...
                for (texture, rect) in textures {
                    for damage_rect in damage
                        .iter()
                        .filter_map(|dmg_rect| dmg_rect.intersection(rect))
                    {
                        let dst = damage_rect
                            .to_logical(1, Transform::Normal, &buffer_size)
                            .to_physical(1);
                        let src =
                            Rectangle::new(damage_rect.loc - rect.loc, damage_rect.size).to_f64();
...
impl MultiTexture {
        ...
        <<A::Device as ApiDevice>::Renderer as RendererSuper>::TextureId: 'static,
    {
        ...
        tex.textures.insert(
            render_id,
            GpuSingleTexture::Direct(Box::new(texture) as Box<_>),
        );
    }
        ...
        <<T::Device as ApiDevice>::Renderer as ExportMem>::TextureMapping: 'static,
    {
        ...
        let old_texture = old_texture.filter(|tex| {
            <dyn Any>::downcast_ref::<
                <<R::Device as ApiDevice>::Renderer as RendererSuper>::TextureId,
            >(tex)
            .map(|tex| tex.size())
                == Some(size)
        });
                ...
                .then(|| {
                    render
                        ...
                        .inspect_err(|err| {
                            debug!(
                                ?err,
                                "failed to import dmabuf on render node {0}",
                                render.node()
                            )
                        })
                        .ok()
                    ...
                    .then(|| {
                        target
                            ...
                            .inspect_err(|err| {
                                debug!(
                                    ?err,
                                    "failed to import dmabuf on target node {0}",
                                    target.node()
                                )
                            })
                            .ok()
                    ...
                    .then(|| {
                        other
                            ...
                            .inspect_err(|err| {
                                debug!(
                                    ?err,
                                    "failed to import dmabuf on other node {0}",
                                    other.node()
                                )
                            })
                            .ok()
    ...
    <<T::Device as ApiDevice>::Renderer as RendererSuper>::TextureId: 'static,
{
    if target.as_ref().is_some_and(|target| {
        !target.can_do_cross_device_imports() || !src.should_do_cross_device_exports()
    }) {
        return Err(Error::ImportFailed);
    }
    ...
    let ((shadow_buffer, _, existing_sync_point), is_new_buffer) = if slot.is_some() {
        ...
        let write_formats =
            Bind::<Dmabuf>::supported_formats(src.renderer()).ok_or(Error::ImportFailed)?;
    ...
    <T::Device as ApiDevice>::Renderer: Renderer + ImportMem,
{
    ...
    let texture_rect =
        Rectangle::from_size((src_texture.width() as i32, src_texture.height() as i32).into());
            ...
            Some(mut mappings) => {
                ...
                let new_damage = damage
                    ...
                    .filter(|rect| {
                        !mappings
                            .iter()
                            .any(|(_, region)| region.contains_rect(**rect))
                    })
                    .copied()
                    .collect::<Vec<_>>();
    ...
    <<T::Device as ApiDevice>::Renderer as RendererSuper>::TextureId: 'static,
{
    ...
    match target_texture.take() {
        ...
        Some(GpuSingleTexture::Dma {
            ...
            let res =
                dma_shadow_copy::<S, T>(src_texture, damage, &mut slot, src, Some(target), None);
        ...
        None => {
            ...
            match dma_shadow_copy::<S, T>(src_texture, damage, &mut slot, src, Some(target), None) {
                ...
                Err(err) => {
                    ...
                    if !ExportMem::can_read_texture(src.renderer_mut(), src_texture)
                        .map_err(Error::Render)?
                    {
                        let mut slot = None;
                        dma_shadow_copy::<S, S>(src_texture, damage, &mut slot, src, None, None)
                            ...
                            (
                                *src.node(),
                                mappings
                                    .into_iter()
                                    .map(|(mapping, damage)| {
                                        (damage, mapping as Box<dyn Any + 'static>)
                                    })
                                    .collect(),
                            )
                        }),
...
where
    ...
    <<<R as GraphicsApi>::Device as ApiDevice>::Renderer as RendererSuper>::TextureId: 'static,
    T: GraphicsApi + 'static,
    <<T as GraphicsApi>::Device as ApiDevice>::Renderer:
        Renderer + ImportDma + ExportMem + Bind<Dmabuf>,
    <<<T as GraphicsApi>::Device as ApiDevice>::Renderer as RendererSuper>::TextureId: 'static,
{
    ...
    <B::Device as ApiDevice>::Renderer: ExportMem,
{
    ...
    fn format(&self) -> Fourcc {
        match self {
            MultiTextureMapping::<A, B>(TextureMappingInternal::Either(x)) => {
                TextureMapping::format(x)
            }
...
impl<'a, 'render, 'target, 'frame, 'buffer, R: GraphicsApi, T: GraphicsApi>
    FrameContext<
        ...
        'frame,
        'buffer,
        LocalMultiRenderer<'a, 'render, 'target, 'frame, 'buffer, R, T>,
        ...
        FrameContext<'a, 'frame, 'buffer, <R::Device as ApiDevice>::Renderer>,
{
    fn blit(
        ...
        // in other words the local multirenderer doesn't let you export a non-local framebuffer created from the non-local multirenderer
        // through type bounds anyway.
        let (MultiFramebufferInternal::Render(from_fb) | MultiFramebufferInternal::Target(from_fb)) =
            &from.0;
        let (MultiFramebufferInternal::Render(to_fb) | MultiFramebufferInternal::Target(to_fb)) =
            &mut to.0;
