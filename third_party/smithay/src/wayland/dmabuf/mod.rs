...
impl DmabufFeedbackBuilder {
    ...
    pub fn build(mut self) -> Result<DmabufFeedback, std::io::Error> {
        ...
        for duplicate_main_tranche in self.preferred_tranches.iter().filter(|tranche| {
            ...
            self.main_tranche.indices = self
                .main_tranche
                .indices
                .sub(&duplicate_main_tranche.indices);
        }
...
struct SurfaceDmabufFeedbackStateInner {
    ...
    known_instances:
        Vec<wayland_server::Weak<zwp_linux_dmabuf_feedback_v1::ZwpLinuxDmabufFeedbackV1>>,
}
...
struct DmabufGlobalState {
    ...
    known_default_feedbacks: Arc<
        Mutex<Vec<wayland_server::Weak<zwp_linux_dmabuf_feedback_v1::ZwpLinuxDmabufFeedbackV1>>>,
    >,
}
...
impl DmabufState {
    ...
    where
        D: GlobalDispatch<zwp_linux_dmabuf_v1::ZwpLinuxDmabufV1, DmabufGlobalData>
            ...
            + 'static,
    {
        self.create_global_with_filter_and_default_feedback::<D, _>(
            display,
            default_feedback,
            |_| true,
        )
        ...
        F: for<'c> Fn(&'c Client) -> bool + Send + Sync + 'static,
    {
            ...
            .fold(
                ...
                |mut formats, format| {
                    if let Some(modifiers) = formats.get_mut(&format.code) {
                        ...
                        formats.insert(
                            format.code,
                            IndexSet::from_iter(std::iter::once(format.modifier)),
                        );
                    }
                    formats
                },
        ...
        let global =
            display.create_global::<D, zwp_linux_dmabuf_v1::ZwpLinuxDmabufV1, _>(version, data);
        self.globals.insert(
            id,
            DmabufGlobalState {
                id: global,
    ...
    pub fn set_default_feedback(&self, global: &DmabufGlobal, default_feedback: &DmabufFeedback) {
        ...
        let Some(mut current_feedback) =
            global.default_feedback.as_ref().map(|f| f.lock().unwrap())
        else {
            return;
        };
        ...
        for feedback in known_default_feedbacks
            .iter()
            .filter_map(|f| f.upgrade().ok())
        {
