...
use crate::{
    backend::{
        allocator::{
            Buffer,
            dmabuf::{
                Dmabuf, DmabufMapping, DmabufMappingMode, DmabufSyncFailed, DmabufSyncFlags,
                WeakDmabuf,
            },
...
use super::{
    ...
    Renderer, RendererSuper, Texture, TextureFilter, TextureMapping, sync::SyncPoint,
};
...
impl Texture for PixmanTarget<'_> {
    ...
    fn size(&self) -> Size<i32, BufferCoords> {
        match &self.0 {
            ...
            PixmanTargetInternal::Image(image) => {
    ...
    fn accessor<'l>(&'l self) -> Result<TextureAccessor<'l>, PixmanError> {
        let guard = if let Some(mapping) = self.0.dmabuf.as_ref() {
            let dmabuf = mapping
                .dmabuf
                .upgrade()
                .ok_or(PixmanError::BufferDestroyed)?;
        ...
        F: for<'a> FnOnce(&'a mut Image<'static, 'static>) -> R,
    {
        ...
        if let Some(buffer) = self.buffer.as_ref() {
            ...
            return shm::with_buffer_contents(buffer, move |ptr, len, data| {
                if unsafe { ptr.offset(data.offset as isize) as *mut u32 }
                    != unsafe { image.data() }
                {
    ...
    fn render_texture_from_to(
        ...
        src_image_accessor.with_image(|src_image| {
            ...
            let mask = if alpha != 1f32 {
                Some(
                    pixman::Solid::new([0f32, 0f32, 0f32, alpha])
...
impl PixmanRenderer {
    ...
    pub fn new() -> Result<Self, PixmanError> {
        let tint =
            pixman::Solid::new([0.0, 0.2, 0.0, 0.2]).map_err(|_| PixmanError::Unsupported)?;
        Ok(Self {
...
impl ImportMem for PixmanRenderer {
    ...
    fn import_memory(
        ...
        let format = pixman::FormatCode::try_from(format)
            .map_err(|_| PixmanError::UnsupportedPixelFormat(format))?;
...
impl ExportMem for PixmanRenderer {
    ...
    fn copy_framebuffer(
        ...
        let format_code = pixman::FormatCode::try_from(format)
            .map_err(|_| PixmanError::UnsupportedPixelFormat(format))?;
        let mut copy_image = pixman::Image::new(
            format_code,
...
impl Bind<Dmabuf> for PixmanRenderer {
    ...
    fn bind<'a>(&mut self, target: &'a mut Dmabuf) -> Result<PixmanTarget<'a>, Self::Error> {
        ...
        let image = if let Some(image) = existing_image {
            ...
            let image =
                self.import_dmabuf(target, DmabufMappingMode::READ | DmabufMappingMode::WRITE)?;
...
impl Offscreen<Image<'static, 'static>> for PixmanRenderer {
    ...
    fn create_buffer(
        ...
        let format_code = FormatCode::try_from(format)
            .map_err(|_| PixmanError::UnsupportedPixelFormat(format))?;
...
impl Bind<Image<'static, 'static>> for PixmanRenderer {
    ...
    ) -> Result<PixmanTarget<'a>, Self::Error> {
...
impl<'a, 'frame, 'buffer> FrameContext<'a, 'frame, 'buffer, PixmanRenderer>
    for PixmanFrame<'frame, 'buffer>
where
    'frame: 'a,
{
