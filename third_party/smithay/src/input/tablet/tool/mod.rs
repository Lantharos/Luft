...
impl<D: TabletSeatHandler + 'static> TabletToolHandle<D> {
    ...
    pub fn proximity_in(
        ...
        if inner.in_proximity() {
            tracing::warn!(
                "proximity_in was called, but the tool is already in proximity with a tablet."
            );
    ...
    pub fn proximity_out(&self, data: &mut D, event: &ProximityOutEvent) {
        ...
        if !inner.in_proximity() {
            tracing::warn!(
                "proximity_out was called, but the tool hasn't entered tablet proximity."
            );
            return;
        }
...
impl<D: TabletSeatHandler + 'static> TabletToolInnerHandle<'_, D> {
    ...
    pub fn current_focus(
        &self,
    ) -> Option<(<D as TabletSeatHandler>::ToolFocus, Point<f64, Logical>)> {
    ...
    pub fn proximity_out(&mut self, data: &mut D, event: &ProximityOutEvent) {
        self.inner
            .proximity_out(data, self.seat, self.descriptor, event);
    }
    ...
    pub fn motion(
        ...
        self.inner
            .motion(data, self.seat, self.descriptor, focus, event);
    }
