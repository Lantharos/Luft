...
use crate::backend::egl::{
    EGLDisplay, EGLDisplayHandle, Error, ffi, wrap_egl_call, wrap_egl_call_bool, wrap_egl_call_ptr,
};
...
impl EGLFence {
    ...
    pub fn create(display: &EGLDisplay) -> Result<Self, Error> {
        ...
        let handle = wrap_egl_call_ptr(|| unsafe {
            ffi::egl::CreateSync(**display_handle, type_, std::ptr::null())
        })
        .map_err(Error::CreationFailed)?;
    ...
    pub fn wait(&self, display: &EGLDisplay) -> Result<(), Error> {
        ...
        wrap_egl_call_bool(|| unsafe {
            ffi::egl::WaitSync(**self.0.display_handle, self.0.handle, 0)
        })
        .map_err(Error::CreationFailed)?;
        Ok(())
    }
