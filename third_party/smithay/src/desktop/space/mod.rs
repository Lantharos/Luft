...
impl<E: SpaceElement + PartialEq> Space<E> {
        ...
        P: Into<Point<i32, Logical>>,
    {
        ...
        {
            let (inner, index) = if let Some(pos) = self
                .elements
                .iter()
                .position(|inner| inner.element == element)
            {
                ...
                (
                    self.create_inner_element(element, location),
                    reference_pos + 1,
                )
            };
            ...
            From<Wrap<<E as AsRenderElements<R>>::RenderElement>>,
    {
        ...
        let mut space_elements: Vec<SpaceElements<'a, E>> = self
            .elements
            .iter()
            .rev()
            .map(SpaceElements::Element)
            .collect();

        #[cfg(feature = "wayland_frontend")]
        {
            let layer_map = layer_map_for_output(output);
            space_elements.extend(layer_map.layers().rev().cloned().map(|l| {
                SpaceElements::Layer {
                    surface: l,
        ...
        From<Wrap<<E as AsRenderElements<R>>::RenderElement>>,
{
    ...
    let mut render_elements: Vec<
        OutputRenderElements<'a, R, <E as AsRenderElements<R>>::RenderElement, C>,
