...
impl X11Surface {
    ...
    pub fn configure_with_sync(
        ...
        if self.is_override_redirect() {
            ...
            if rect.size == state.last_configure.size && state.pending_configure.is_none() {
                ...
                match self
                    .send_sync_request(&mut state, timeout.unwrap_or(DEFAULT_SYNC_REQUEST_TIMEOUT))
                {
    ...
    fn send_sync_request(
            ...
            .filter(|_| self.servertime_counter.is_some())
        {
            if state.pending_sync_wait_value.is_none() {
                ...
                let event =
                    ClientMessageEvent::new(32, self.window, self.atoms.WM_PROTOCOLS, msg_data);
    ...
    fn init_sync_alarm(
        &self,
        state: &mut MutexGuard<'_, SharedSurfaceState>,
    ...
    pub(super) fn handle_sync_alarm(&self, new_value: Int64) -> bool {
        ...
        state.next_counter_value =
            value_to_xsync(new_value_raw.max(xsync_value(state.next_counter_value)));
    ...
    pub(crate) fn set_wl_surface<D: SeatHandler + 'static>(
        &self,
        data: &mut D,
    ...
    pub fn window_type(&self) -> Option<WmWindowType> {
        self.state
            ...
            .find_map(|atom| match atom {
                ...
                x if *x == self.atoms._NET_WM_WINDOW_TYPE_DROPDOWN_MENU => {
    ...
    pub(super) fn update_property(
        &self,
        atom: Atom,
    ) -> Result<Option<WmWindowProperty>, ConnectionError> {
        match atom {
    ...
    fn update_normal_hints(&self) -> Result<(), ConnectionError> {
        ...
        let mut state = self.state.lock().unwrap();
        state.normal_hints =
            match WmSizeHints::get_normal_hints(&*conn, self.window)?.reply_unchecked() {
                Ok(hints) => hints,
                Err(ConnectionError::ParseError(_)) => None,
                Err(err) => return Err(err),
            };
    ...
    fn fetch_gtk_frame_extents(
        &self,
    ) -> Result<Option<FrameExtents<i32, Physical>>, ConnectionError> {
        let conn = self.conn.upgrade().ok_or(ConnectionError::UnknownError)?;
        conn.get_property(
            false,
            self.window,
    ...
    fn read_window_property_string(
        ...
        match reply.type_ {
            x if x == AtomEnum::STRING.into() => {
                Ok(Some(WINDOWS_1252.decode(&bytes).0.to_string()))
            }
    ...
    pub fn send_ping(&self, timestamp: u32) -> Result<(), PingError> {
        if timestamp == x11rb::CURRENT_TIME {
            ...
            if let Some(timestamp) = &state.pending_ping_timestamp {
                ...
                let conn = self.conn.upgrade().ok_or(ConnectionError::UnknownError)?;
                if state.protocols.contains(&WMProtocol::NetWmPing)
                ...
                values
                    ...
                    .flat_map(|rect_values| {
                        let phys_rect = Rectangle::<i32, Physical>::new(
                            ...
                            (
                                (rect_values[2] as i32).max(0),
                                (rect_values[3] as i32).max(0),
                            )
                                .into(),
                        );
