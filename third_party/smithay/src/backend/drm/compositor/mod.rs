    ...
    backend::{
        ...
        renderer::{
            ...
            element::{
                ...
                RenderElementState, RenderElementStates, RenderingReason, UnderlyingStorage,
            },
            sync::SyncPoint,
    ...
    fn test_state(
        ...
        *current_config = state;
        ...
        let res = surface.test_state(
            self.build_planes(surface, supports_fencing, true),
            allow_modeset,
        );
...
type CompositorFrameState<A, F> = FrameState<
    ...
    G: AsFd + 'static,
{
    ...
    previous_element_states:
        IndexMap<Id, ElementState<<F as ExportFramebuffer<A::Buffer>>::Framebuffer>>,
    ...
    G: AsFd + Clone,
{
                    ...
                    let cursor_state = gbm.map(|gbm| {
                        ...
                        let pixman_renderer = match PixmanRenderer::new() {
                            ...
                            Err(err) => {
                                tracing::warn!(
                                    ?err,
                                    "failed to initialize pixman renderer for cursor plane"
                                );
                                None
                            }
                        };
    ...
    fn test_format(
        ...
        buffer.userdata().insert_if_missing_threadsafe(|| {
    ...
    fn find_supported_format(
        ...
        let formats = {
            ...
            if (plane_formats.len() == 1
                ...
                && renderer_formats
                    .iter()
                    .any(|x| x.modifier == DrmModifier::Linear))
        ...
        R::TextureId: Texture + 'static,
    {
        ...
        let dmabuf = primary_plane_buffer
            .export()
            .map_err(FrameError::AsDmabufError)?;
        ...
        let mut element_opaque_regions_workhouse =
            std::mem::take(&mut self.element_opaque_regions_workhouse);
        for (index, element) in elements.iter().enumerate() {
            ...
            let element_visible_area = element_opaque_regions_workhouse
                ...
                .fold(0usize, |acc, item| {
                    acc + (item.size.w * item.size.h) as usize
                });
            ...
            output_elements.iter().enumerate()
        {
            ...
            let try_assign_primary_plane = if remaining_elements == 1
                ...
                let overlaps_with_underlay = self
                    ...
                    .filter(|p| {
                        p.zpos.unwrap_or_default()
                            < self.surface.plane_info().zpos.unwrap_or_default()
                    })
        ...
        if render {
            ...
            let (mut dmabuf, age) = {
                let primary_plane_state =
                    next_frame_state.plane_state(self.surface.plane()).unwrap();
            ...
            let overlay_plane_elements =
                overlay_plane_elements.iter().filter_map(|(p, element)| {
                    let id = self
                        .overlay_plane_element_ids
                    ...
                    if is_underlay {
                        Some(
                            HolepunchRenderElement::from_render_element(id, element, output_scale)
                                .into(),
                        )
                    } else {
            ...
            match render_res {
                Ok(render_output_result) => {
                    ...
                    if !had_direct_scan_out {
                        ...
                        self.primary_plane_damage_bag.add([output_geometry
                            .to_logical(1)
                            .to_buffer(1, Transform::Normal, &output_geometry.size.to_logical(1))]);
        ...
        let frame_reference: RenderFrameResult<'a, A::Buffer, F::Framebuffer, E> =
            RenderFrameResult {
    ...
    pub fn queue_frame(&mut self, user_data: U) -> FrameResult<(), A, F> {
        ...
        if let Some(plane_state) = prepared_frame.frame.plane_state(self.surface.plane()) {
            if !plane_state.skip {
                let slot = plane_state
                    .buffer()
                    .and_then(|config| match &config.buffer {
    ...
    fn try_assign_element<'a, R, E>(
        ...
        element_states: &mut IndexMap<
            Id,
            ElementState<<F as ExportFramebuffer<A::Buffer>>::Framebuffer>,
        >,
        ...
        E: RenderElement<R>,
    {
        ...
        if element_size.w > self.cursor_size.w || element_size.h > self.cursor_size.h {
            trace!(
                "element {:?} too big for cursor plane(s), skipping",
                element.id(),
            );
            return None;
        }
        ...
        if !render && !reposition {
            let mut plane_state = previous_state
                .plane_state(plane_info.handle)
                .unwrap()
                .clone();
        ...
        let cursor_buffer_size = cursor_plane_size
            .to_logical(1)
        ...
        if !copy_element_to_cursor_bo(
            ...
            tracing::trace!(
                "failed to copy element to cursor bo, skipping element on cursor plane"
            );
            return None;
        }
                ...
                .map_mut::<_, Result<_, PixmanError>>(
                    ...
                    |mbo| {
                        ...
                        frame.clear(
                            ...
                            &[Rectangle::from_size(cursor_plane_size)],
                        )?;
                        let src = element.src();
        ...
        E: RenderElement<R>,
    {
        ...
        if !self
            .framebuffer_exporter
            .can_add_framebuffer(&export_buffer)
        {
        ...
        let element_cache_key = ElementFramebufferCacheKey::from_underlying_storage(
            &underlying_storage,
            allow_opaque_fallback,
        )
        .ok_or(ExportBufferError::Unsupported)?;
                ...
                .map(|state| {
                    state
                        ...
                        && state
                            ...
                            .as_ref()
                            .map(|config| {
                                config.properties.is_compatible(&element_config.properties)
                            })
                            .unwrap_or(false)
                })
                .unwrap_or(false)
        ...
        E: RenderElement<R>,
    {
        ...
        let plane_claim = match self.surface.claim_plane(plane.handle) {
            ...
            None => {
                trace!(
                    "failed to claim {:?} for element {:?}",
                    plane.handle, element_id
                );
                return Err(None);
            }
        };
    ...
    match underlying_storage {
        UnderlyingStorage::Wayland(buffer) => {
            ...
            shm::with_buffer_contents(buffer, |ptr, len, data| {
                ...
                copy_to_bo(
                    unsafe {
                        std::slice::from_raw_parts(ptr.offset(data.offset as isize), expected_len)
                    },
                    data.stride,
                    data.height,
                )
...
impl<B: Framebuffer + std::fmt::Debug> std::fmt::Debug for CachedDrmFramebuffer<B> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_tuple("CachedDrmFramebuffer")
            .field(&self.0)
            .finish()
    }
}
