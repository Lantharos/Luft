    ...
    crate::backend::SwapBuffersError: From<R::Error>,
{
    ...
    pub fn bind(
        &mut self,
    ) -> Result<(&mut R, R::Framebuffer<'_>), crate::backend::SwapBuffersError> {
...
impl WinitGraphicsBackend<GlesRenderer> {
    /// Bind the underlying window and retrieve the age of that exact backbuffer.
    #[instrument(level = "trace", parent = &self.span, skip(self))]
    #[profiling::function]
    pub fn bind_with_buffer_age(
        &mut self,
    ) -> Result<(&mut GlesRenderer, GlesTarget<'_>, usize), crate::backend::SwapBuffersError> {
        let window_size = self.window_size();
        if Some(window_size) != self.bind_size {
            self.egl_surface.resize(window_size.w, window_size.h, 0, 0);
        }
        ...
        let (framebuffer, buffer_age) = self
            .renderer
            .bind_egl_with_buffer_age(&mut self.egl_surface)?;
        let buffer_age = if self.damage_tracking { buffer_age } else { 0 };

        Ok((&mut self.renderer, framebuffer, buffer_age))
    }
}
