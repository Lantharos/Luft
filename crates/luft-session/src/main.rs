use clap::Parser;
use luft_config::{BackendPreference, ConfigPaths, ConfigSource, load_config};
use luft_session::{SessionDescriptor, SessionEnvironment, validate_descriptor};
use std::{
    env, fs,
    fs::OpenOptions,
    io,
...
fn resolve_kestrel(explicit: Option<PathBuf>) -> PathBuf {
    ...
    if let Some(path) = env::var_os("LUFT_KESTREL") {
...
fn init_logging() {
    let subscriber = tracing_subscriber::fmt()
        .with_ansi(false)
        .with_writer(file_log_writer("luft-session"))
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| tracing_subscriber::EnvFilter::new("luft_session=info")),
        )
        .finish();
...
mod tests {
    ...
    fn shell_escape_quotes_values_with_spaces() {
        assert_eq!(shell_escape("/tmp/Luft Kestrel"), "'/tmp/Luft Kestrel'");
    }
