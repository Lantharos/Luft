use crate::{
    ...
    wayland::{
        ...
        seat::WaylandFocus,
        shell::xdg::{
            PopupCachedState, XDG_POPUP_ROLE, XdgPopupSurfaceData, XdgPopupSurfaceRoleAttributes,
        },
    },
};
...
impl PopupManager {
    ...
    fn add_popup(&mut self, popup: PopupKind) -> Result<(), DeadResource> {
        ...
        with_states(&root, |states| {
            ...
            if states
                .data_map
                .insert_if_missing_threadsafe(|| tree.clone())
            {
                trace!(
                    "Adding popup {:?} to new PopupTree on root {:?}",
                    popup, root
                );
                tree.insert(popup);
