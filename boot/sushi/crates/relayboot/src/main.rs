...
const RELAY_VENDOR_GUID: uefi::Guid = uefi::guid!("a7b3c4d5-e6f7-4890-abcd-ef1234567890");

// Must match relay-core spinner constants exactly.
...
fn efi_main() -> Status {
    ...
    boot_log(&format!("RelayBoot: GOP {}x{}", scene.width, scene.height));
    ...
    store_relay_state(&scene);
    let entry = if entries.is_empty() {
        BlsEntry {
            title: String::from("Relay Linux"),
            linux: String::from("vmlinuz"),
            initrd: Some(String::from("initramfs.img")),
            options: String::from("rd.relay=1"),
        }
    } else {
        entries[0].clone()
    };

    boot_log(&format!("RelayBoot: booting {}", entry.title));
    ...
    match boot_linux_image(device, &linux) {
        Ok(()) => Status::SUCCESS,
        Err(err) => {
            boot_log(&format!("RelayBoot: boot failed: {err:?}"));
            Status::LOAD_ERROR
        }
    }
}
...
fn store_relay_state(scene: &BootScene) {
    let json = format!(
        concat!(
    ...
    if let Ok(addr) = boot::allocate_pages(AllocateType::AnyPages, MemoryType::LOADER_DATA, pages) {
        ...
        unsafe {
            ...
            let _ = boot::install_configuration_table(&RELAY_VENDOR_GUID, ptr.cast());
        }
    }
}
