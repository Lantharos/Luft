...
use super::{
    AlreadyHasRole, BufferAssignment, CompositorHandler, Damage, Rectangle, RectangleKind,
        ...
        match request {
            wl_subcompositor::Request::GetSubsurface {
                ...
                super::with_states(&surface, |states| {
                    states
                        .data_map
                        .insert_if_missing_threadsafe(SubsurfaceState::new)
                });
    ...
    fn request(
        ...
        match request {
            ...
            wl_subsurface::Request::PlaceAbove { sibling } => {
                if let Err(()) =
                    PrivateSurfaceData::reorder(&self.surface, Location::After, &sibling)
                {
            ...
            wl_subsurface::Request::PlaceBelow { sibling } => {
                if let Err(()) =
                    PrivateSurfaceData::reorder(&self.surface, Location::Before, &sibling)
                {
                    subsurface.post_error(
                        wl_subsurface::Error::BadSurface,
                        "Provided surface is not a sibling or parent.",
