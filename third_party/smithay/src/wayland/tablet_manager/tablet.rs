...
use wayland_server::{
    Client, Dispatch, DisplayHandle, Resource, Weak, backend::ObjectId,
    protocol::wl_surface::WlSurface,
};

use crate::{
    input::tablet::{
        Tablet, TabletDescriptor, TabletRc, TabletSeat, TabletSeatHandler, WeakTablet,
    },
    wayland::Dispatch2,
};
        ...
        D: 'static,
    {
        ...
        for seat in instances.iter() {
            ...
            if let Ok(client) = dh.get_client(seat.id()) {
                tablet.arc.wp_tablet.new_instance::<D>(
                    &client,
                    dh,
                    &seat,
                    tablet.clone(),
...
pub(crate) struct WpTabletHandle {
    bound: bool,
    known_instances:
        Mutex<Vec<Weak<wayland_protocols::wp::tablet::zv2::server::zwp_tablet_v2::ZwpTabletV2>>>,
}
