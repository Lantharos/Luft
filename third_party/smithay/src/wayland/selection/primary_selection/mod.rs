    ...
    D: SeatHandler + PrimarySelectionHandler + 'static,
{
    ...
    let seat_data = seat
        ...
        .get::<RefCell<SeatData<D::SelectionUserData>>>()
        .unwrap();
    Ref::filter_map(seat_data.borrow(), |data| {
        match data.get_primary_selection() {
            Some(OfferReplySource::Compositor(CompositorSelectionProvider {
                user_data, ..
            })) => Some(user_data),
            _ => None,
        }
...
mod handlers {
    ...
    use wayland_protocols::wp::primary_selection::zv1::server::{
        zwp_primary_selection_device_manager_v1::{
            self as primary_device_manager,
            ZwpPrimarySelectionDeviceManagerV1 as PrimaryDeviceManager,
        },
    ...
    use super::{
        PrimaryDeviceManagerGlobalData, device::PrimaryDeviceUserData,
        source::PrimarySourceUserData,
    };
                    ...
                    match Seat::<D>::from_resource(&wl_seat) {
                        Some(seat) => {
                            seat.user_data().insert_if_missing(|| {
                                RefCell::new(SeatData::<D::SelectionUserData>::new())
                            });
