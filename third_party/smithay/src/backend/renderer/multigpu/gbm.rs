...
use crate::{
    backend::{
        ...
        renderer::{
            ...
            multigpu::{
                Error as MultigpuError, MultiRenderer, MultiTexture, import_dmabuf_internal,
            },
        },
    },
        ...
        let new_renderers = self
            ...
            .map(|(node, (display, gbm))| {
                let renderer = if let Some(factory) = self.factory.as_ref() {
                    ...
                    let context = EGLContext::new_with_priority(
                        display,
                        self.context_priority.unwrap_or_default(),
                    )
                    .map_err(Error::Egl)?;
        ...
        if let Some(dmabuf) = Self::try_import_egl(self.render.renderer_mut(), buffer)
            ...
            .or_else(|| {
                self.target.as_mut().and_then(|renderer| {
                    Self::try_import_egl(renderer.device.renderer_mut(), buffer).ok()
                })
            })
            .or_else(|| {
            ...
            })
        {
            ...
            if res.is_ok() {
                if let Some(surface) = surface {
                    surface
                        .data_map
                        .insert_if_missing_threadsafe(|| texture_ref);
                }
            }
            return res;
        }
