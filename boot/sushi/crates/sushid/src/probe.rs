...
use sushi::{DisplayError, DisplayManager};
use sushi::DrmBackend;
use sushi::FbdevBackend;
...
pub fn probe_and_acquire() -> Result<DisplayManager, DisplayError> {
    ...
    if let Ok(entries) = fs::read_dir("/dev") {
        let mut names: Vec<_> = entries
            ...
            .collect();
        names.sort();
        eprintln!("sushid: display-related /dev nodes: {names:?}");
    }
    ...
    for path in ["/dev/fb0", "/dev/fb1"] {
        if !Path::new(path).exists() {
            ...
            eprintln!("sushid: {path} missing (sysfs exists={})", Path::new(&sysfs).exists());
            continue;
        }
        match FbdevBackend::open(path) {
            Ok(backend) => {
                eprintln!("sushid: using framebuffer {path}");
                return Ok(DisplayManager::from_backend(Box::new(backend)));
            }
            Err(err) => eprintln!("sushid: framebuffer {path} unavailable: {err:#}"),
        }
    }

    // DRM last: in QEMU/VGA setups fbdev maps straight to the visible scanout buffer.
    if let Ok(backend) = DrmBackend::probe() {
        eprintln!("sushid: using DRM backend (fallback)");
        return Ok(DisplayManager::from_backend(Box::new(backend)));
    }
