                ...
                if offer.last_source_actions != new_metadata.dnd_actions {
                    for wl_offer in &offer.wl_offers {
                        if wl_offer.version() >= wl_data_offer::EVT_SOURCE_ACTIONS_SINCE {
                            wl_offer.source_actions(DndAction::convert_slice(
                                &new_metadata.dnd_actions,
                            ));
                        }
                    }
...
pub fn current_data_device_selection_userdata<D>(
    seat: &Seat<D>,
    ...
    D: SeatHandler + DataDeviceHandler + 'static,
{
    ...
    Ref::filter_map(seat_data.borrow(), |data| {
        match data.get_clipboard_selection() {
    ...
    D: SeatHandler + DataDeviceHandler + 'static,
{
    ...
    seat_data
        .borrow_mut()
        .set_clipboard_selection::<D>(dh, None);
}
