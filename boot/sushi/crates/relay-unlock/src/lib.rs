...
use relay_core::{RelayEvent, RelayEventKind, RelayStage, VisualMode};
use relay_log;
use thiserror::Error;

pub const ASK_PASSWORD_DIR: &str = "/run/systemd/ask-password";
pub const RELAY_ASK_AGENT_PID: &str = "/run/relay/ask-agent.pid";
...
impl AskPasswordAgent {
    pub fn start() -> Result<(Self, Receiver<UnlockMessage>)> {
        fs::create_dir_all("/run/relay").context("create /run/relay")?;
        let (tx, rx) = mpsc::channel();
        let shutdown = Arc::new(AtomicBool::new(false));
        let worker_shutdown = shutdown.clone();
        let handle = thread::spawn(move || watch_ask_password(tx, worker_shutdown));
        fs::write(RELAY_ASK_AGENT_PID, std::process::id().to_string())?;
    ...
    pub fn shutdown(&mut self) {
        ...
        let _ = fs::remove_file(RELAY_ASK_AGENT_PID);
    }
}
...
pub fn log_unlock_event(kind: RelayEventKind) {
    relay_log::log_event(&RelayEvent::new(RelayStage::Initramfs, kind));
}

pub fn enter_unlock_mode() -> RelayEvent {
    log_unlock_event(RelayEventKind::UnlockManualRequired);
    RelayEvent::new(RelayStage::Initramfs, RelayEventKind::UnlockManualRequired)
}

pub fn unlock_failed() -> RelayEvent {
    log_unlock_event(RelayEventKind::UnlockManualFailed);
    RelayEvent::new(RelayStage::Initramfs, RelayEventKind::UnlockManualFailed)
}
