...
impl MemoryBuffer {
    ...
    pub fn resize(&mut self, size: impl Into<Size<i32, Buffer>>) -> bool {
        self.size = size.into();
        self.stride = self.size.w
            * (get_bpp(self.format).expect("Format with unknown bits per pixel") / 8) as i32;
        ...
        R::TextureId: Send + Clone + 'static,
    {
        ...
        let tex = match self.textures.entry(context_id.clone()) {
            ...
            Entry::Vacant(entry) => {
                ...
                let tex =
                    renderer.import_memory(&self.mem, self.mem.format(), self.mem.size(), false)?;
...
impl MemoryRenderBuffer {
    ...
    pub fn from_slice(
        ...
        let inner = MemoryRenderBufferInner::from_slice(
            mem,
            format,
            size,
            scale,
...
impl<R: Renderer> MemoryRenderBufferRenderElement<R> {
    ...
    fn scale(&self) -> Scale<f64> {
        let src = self.src();
        Scale::from((
            self.size.w as f64 / src.size.w,
            self.size.h as f64 / src.size.h,
        ))
    }
}
