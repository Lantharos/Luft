...
impl EventSource for EiInput {
        ...
        F: FnMut(EiInputEvent, &mut EiInputConnection),
    {
        let _ = self
            .channel_source
            .process_events(readiness, token, |event, ()| {
        ...
        self.source
            .process_events(readiness, token, |event, connection| {
                ...
                match event {
                    Ok(EisRequestSourceEvent::Connected) => {
                        if connection.0.connection.context_type()
...
fn convert_request(request: EisRequest) -> Option<InputEvent<EiInput>> {
    match request {
        ...
        EisRequest::PointerMotionAbsolute(event) => {
            Some(InputEvent::PointerMotionAbsolute { event })
        }
        ...
        EisRequest::DeviceClosed(event) => Some(InputEvent::DeviceRemoved {
            device: event.device,
        }),
