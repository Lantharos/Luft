...
use crate::{
    backend::renderer::{
        element::{
            Id, PrimaryScanoutOutput, RenderElementPresentationState, RenderElementState,
            RenderElementStates,
        },
...
impl RendererSurfaceState {
    fn contains_point<P: Into<Point<f64, Logical>>>(
        &self,
        ...
        |wl_surface, states, location: &Point<i32, Logical>| {
            ...
            if let Some(surface_view) = data.and_then(|d| d.lock().unwrap().surface_view) {
                ...
                let contains_the_point = data
                    .map(|data| {
                        data.lock().unwrap().contains_point(
                            &*states.cached_state.get().current(),
    ...
    F: FnMut(&wl_surface::WlSurface, &SurfaceData) -> Option<Output>,
{
    ...
    with_surface_tree_downward(
        ...
        |surface, states, &()| {
            ...
            let surface_frame_throttling_state = states
                .data_map
