...
impl VulkanAllocator {
    ...
    pub fn new(
        ...
        let queue_family_index = queue_families
            ...
            .or_else(|| {
                queue_families.iter().position(|properties| {
        ...
        let extension_fns = ExtensionFns {
            ext_image_format_modifier: ext::image_drm_format_modifier::Device::new(
                instance, &device,
            ),
    ...
    pub fn create_buffer_with_usage(
        ...
        Ok(unsafe {
            self.create_image(width, height, vk_format, vk_usage, fourcc, &modifiers[..])
        }?)
    }
...
impl AsDmabuf for VulkanImage {
    ...
    fn export(&self) -> Result<Dmabuf, Self::Error> {
        ...
        for idx in 0..self.format_plane_count {
            ...
            let layout =
                unsafe { device.get_image_subresource_layout(self.inner.image, subresource) };
...
impl VulkanAllocator {
    ...
    unsafe fn create_image(
        ...
        unsafe {
            ...
            self.device
                .bind_image_memory(guard.image, guard.memory, 0)?;
        }
