...
use crate::core::{SushiVisualState, SUSHI_STATE_PATH};
use serde::{Deserialize, Serialize};
use thiserror::Error;

pub const SUSHI_EFI_GUID: &str = "a7b3c4d5-e6f7-4890-abcd-ef1234567890";
pub const SUSHI_EFI_TABLE_NAME: &str = "SushiVisualState";

#[derive(Debug, Error)]
pub enum ProtocolError {
    #[error("unsupported sushi state version: {0}")]
...
pub struct BootDiagnostics {
    ...
    pub events: Vec<crate::core::SushiEvent>,
}
...
impl BootDiagnostics {
    pub fn from_events(events: Vec<crate::core::SushiEvent>, boot_id: String) -> Self {
        ...
        let first_frame_ms = events
            .iter()
            .find(|e| matches!(e.kind, crate::core::SushiEventKind::BootFirstFrame))
            .map(|e| (e.timestamp_ns / 1_000_000) as u64);

        for event in &events {
            match &event.kind {
                crate::core::SushiEventKind::HandoffComplete { .. } => visual_handoffs += 1,
                crate::core::SushiEventKind::HandoffBegin { .. } => {}
                crate::core::SushiEventKind::BootDegraded { .. } => degraded = true,
                crate::core::SushiEventKind::BlackScreenDetected { .. } => {
                    black_screen_events += 1;
                    degraded = true;
                }
                crate::core::SushiEventKind::RecoveryEntered { .. } => {
                    failed_handoffs += 1;
                    degraded = true;
                }
                _ => {}
...
pub fn write_state(path: impl AsRef<Path>, state: &SushiVisualState) -> Result<()> {
    let path = path.as_ref();
    if let Some(parent) = path.parent() {
        fs::create_dir_all(parent).with_context(|| format!("create {}", parent.display()))?;
    }
    let json = serde_json::to_vec_pretty(state).context("serialize sushi state")?;
...
pub fn read_state(path: impl AsRef<Path>) -> Result<SushiVisualState, ProtocolError> {
    let path = path.as_ref();
    let data = fs::read_to_string(path).map_err(|_| ProtocolError::Missing)?;
    let state: SushiVisualState =
        serde_json::from_str(&data).map_err(|_| ProtocolError::Missing)?;
    if state.version != crate::core::SUSHI_STATE_VERSION {
        return Err(ProtocolError::UnsupportedVersion(state.version));
    }
    Ok(state)
}

pub fn load_state() -> Option<SushiVisualState> {
    read_state(SUSHI_STATE_PATH).ok()
}

pub fn persist_state(state: &SushiVisualState) -> Result<()> {
    write_state(SUSHI_STATE_PATH, state)
}

pub fn state_from_efi_payload(payload: &[u8]) -> Option<SushiVisualState> {
    let trimmed = trim_nul(payload);
    if let Ok(mut state) = serde_json::from_slice::<SushiVisualState>(trimmed) {
    ...
    let mut state = SushiVisualState::new_boot_scene(
    ...
    if let Some(flags) = partial.flags {
        state.flags = crate::core::VisualFlags::from_bits_truncate(flags);
    }
...
/// Load visual state handed off from SushiBoot via EFI config table or cmdline.
pub fn load_state_from_efi() -> Option<SushiVisualState> {
    if let Some(payload) = read_efi_config_table(SUSHI_EFI_GUID) {
        if let Some(state) = state_from_efi_payload(&payload) {
            return Some(state);
        }
    }
    if let Some(payload) = read_cmdline_sushi_state() {
...
fn read_cmdline_sushi_state() -> Option<String> {
    let cmdline = fs::read_to_string("/proc/cmdline").ok()?;
    for token in cmdline.split_whitespace() {
        let Some(value) = token.strip_prefix("sushi.state=") else {
            continue;
        };
        return decode_sushi_state_token(value);
    }
    None
}

fn decode_sushi_state_token(value: &str) -> Option<String> {
...
pub fn state_to_efi_payload(state: &SushiVisualState) -> Result<Vec<u8>> {
