...
use x11rb::{
    ...
    protocol::{
        ...
        xinput,
        xproto::{
            ColormapAlloc, ConnectionExt, CreateWindowAux, VisualClass, WindowClass, WindowWrapper,
        },
    },
        ...
        F: FnMut(X11Event, &mut ()),
    {
        ...
        match event {
            ...
            x11::Event::XinputMotion(motion_notify) => {
                if let Some(window) = X11Inner::window_ref_from_id(inner, &motion_notify.event)
                    .and_then(|w| w.upgrade())
                {
            ...
            x11::Event::ConfigureNotify(configure_notify) => {
                if let Some(window) = X11Inner::window_ref_from_id(inner, &configure_notify.window)
                    .and_then(|w| w.upgrade())
                {
            ...
            x11::Event::XinputEnter(enter_notify) => {
                if let Some(window) = X11Inner::window_ref_from_id(inner, &enter_notify.event)
                    .and_then(|w| w.upgrade())
                {
                    window.cursor_enter();
                }
            ...
            x11::Event::XinputLeave(leave_notify) => {
                if let Some(window) = X11Inner::window_ref_from_id(inner, &leave_notify.event)
                    .and_then(|w| w.upgrade())
                {
                    window.cursor_leave();
                }
            ...
            x11::Event::ClientMessage(client_message) => {
                if let Some(window) = X11Inner::window_ref_from_id(inner, &client_message.window)
                    .and_then(|w| w.upgrade())
                {
            ...
            x11::Event::PresentCompleteNotify(complete_notify) => {
                if let Some(window) = X11Inner::window_ref_from_id(inner, &complete_notify.window)
                    .and_then(|w| w.upgrade())
                {
