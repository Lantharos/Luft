...
fn test_gbm_bo_fd_for_plane() {
    let gbm = match pkg_config::probe_library("gbm") {
        ...
        Err(_) => {
            println!(
                "cargo:warning=failed to find gbm, assuming gbm_bo_get_fd_for_plane is unavailable"
            );
            return;
        }
    };
    ...
    let has_gbm_bo_get_fd_for_plane = cc::Build::new()
        .file(
            std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("test_gbm_bo_get_fd_for_plane.c"),
        )
    ...
    let has_gbm_bo_create_with_modifiers2 = cc::Build::new()
        .file(
            std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
                .join("test_gbm_bo_create_with_modifiers2.c"),
        )
        .includes(gbm.include_paths)
        .warnings_into_errors(true)
        .cargo_metadata(false)
