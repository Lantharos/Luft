...
impl CommitTimingManagerState {
    ...
    where
        ...
        D: 'static,
    {
        let global = display.create_global::<D, WpCommitTimingManagerV1, _>(
            1,
            CommitTimingManagerData { is_managed },
        );
    ...
    fn request(
        ...
        match request {
            wp_commit_timing_manager_v1::Request::GetTimer { id, surface } => {
                ...
                let commit_timer: WpCommitTimerV1 =
                    data_init.init(id, CommitTimerData(surface.downgrade()));
            ...
            wp_commit_timer_v1::Request::SetTimestamp {
                ...
                let Ok(surface) = self.0.upgrade() else {
                    resource.post_error(
                        ...
                        "the surface associated with this commit timer object has been destroyed"
                            .to_string(),
                    );
                    return;
                };
                ...
                if already_has_timestamp {
                    resource.post_error(
                        ...
                        "the surface already has a timestamp associated for this commit"
                            .to_string(),
                    );
                }
            }
