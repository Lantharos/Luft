...
impl DrmLeaseState {
        ...
        F: for<'c> Fn(&'c Client) -> bool + Send + Sync + 'static,
    {
        ...
        let global =
            display.create_global::<D, wp_drm_lease_device_v1::WpDrmLeaseDeviceV1, _>(1, data);
    ...
    pub fn withdraw_connector(&mut self, connector: connector::Handle) {
        ...
        if let Some(pos) = self
            .connectors
            .iter()
            .position(|conn| conn.handle == connector)
        {
        ...
        match request {
            ...
            wp_drm_lease_request_v1::Request::Submit { id } => {
                ...
                for connector in &*self.connectors.lock().unwrap() {
                    ...
                    {
                        if connector_handles
                            .iter()
                            .any(|handle| handle == &conn.handle)
                        {
                            resource.post_error(
