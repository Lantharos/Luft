...
The screenshot tool is a single glass bar above the panel: area, screen, and window modes as icons, the photo and video switch, the pointer toggle, the capture button, and close, with a tooltip on each. Selection handles are small dots on the selection outline.
...
Apps that use StatusNotifierItem, such as Discord, Steam, Nextcloud, and 1Password, show their tray icon beside the status icons on the primary display. Colored icons are drawn in monochrome to match the panel; symbolic icons keep their own shapes. Clicking an icon opens the app, or its menu when the app has nothing else to show; the middle button runs the app's secondary action and scrolling is passed to the app. Right-click or the Menu key opens the app's menu, with checkmarks, radio choices, icons, and submenus that open in place with a Back row. Icons follow the app's status: passive items stay hidden and attention icons replace the regular icon. Apps that registered before Kestrel started are found at startup, and icons disappear when their app exits.
