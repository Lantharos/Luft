...
impl Default for SessionDescriptor {
    fn default() -> Self {
        Self {
            name: "Luft".to_string(),
            comment: "Luft Desktop Environment".to_string(),
            exec: "luft-session --session".to_string(),
            try_exec: default_try_exec(),
            desktop_names: "Luft".to_string(),
...
mod tests {
    ...
    const INSTALLED_DESKTOP_ENTRY: &str = include_str!("../../../data/sessions/luft.desktop");
