...
impl<const MIN_TILE_SIDE: i32> DamageShaper<MIN_TILE_SIDE> {
    ...
    fn shape_damage_impl(
        ...
        let (x_min, y_min, x_max, y_max, max_damage_area) = in_damage.iter().fold(
        ...
        let damage_bbox =
            Rectangle::<i32, Physical>::new((x_min, y_min).into(), (bbox_w, bbox_h).into());
    ...
    fn shape_damage_tiled(
        ...
        for x in (bbox.loc.x..bbox.loc.x + bbox.size.w).step_by(tile_size.w as usize) {
            ...
            for idx in num_tiles - tiles_in_column..num_tiles - 1 {
                let (damage, adjacent_damage) = match (
                    self.tiles_cache[idx].damage,
                    self.tiles_cache[idx + 1].damage,
                ) {
                    ...
                    && (damage.size.w - adjacent_damage.size.w).abs() < tile_gap.w
                {
                    ...
                    #[cfg(test)]
                    {
                        self.tiles_cache[idx + 1].merged_tiles =
                            self.tiles_cache[idx].merged_tiles + 1;
                    }
                }
            }
        }
