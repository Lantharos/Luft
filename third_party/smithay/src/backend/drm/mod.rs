...
pub use device::{
    DrmDevice, DrmDeviceFd, DrmDeviceNotifier, DrmEvent, EventMetadata as DrmEventMetadata,
    PlaneClaim, Time as DrmEventTime, WeakDrmDeviceFd,
};
    ...
    if let Ok(1) = dev.get_driver_capability(DriverCapability::AddFB2Modifiers) {
        ...
        if let Some(prop) = prop {
            ...
            let raw_value = raw_values[handles
                ...
                .unwrap()];
            if let drm::control::property::Value::Blob(blob) =
                prop_info.value_type().convert_value(raw_value)
            {
                ...
                unsafe {
                    let fmt_mod_blob_ptr =
                        data.as_ptr() as *const drm_ffi::drm_format_modifier_blob;
    ...
    for (&id, &val) in ids.iter().zip(vals.iter()) {
            ...
            .unwrap_or(false)
        {
                ...
                info.value_type().convert_value(val)
            {
                ...
                let blob_info = drm_ffi::mode::get_property_blob(dev.as_fd(), blob_id as u32, None)
                    .map_err(|source| {
                        DrmError::Access(AccessError {
                ...
                drm_ffi::mode::get_property_blob(dev.as_fd(), blob_id as u32, Some(&mut data))
                    .map_err(|source| {
                        DrmError::Access(AccessError {
                ...
                let size_hints = unsafe {
                    std::slice::from_raw_parts(
                        data.as_ptr() as *const drm_plane_size_hint,
                        num_size_hints,
                    )
                };
                Some(
