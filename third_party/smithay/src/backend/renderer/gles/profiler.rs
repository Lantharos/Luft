...
pub struct SpanLocation(
    #[cfg(feature = "tracy_gpu_profiling")] pub &'static tracy_client::SpanLocation,
);
...
mod imp {
    ...
    impl QueryPool {
        ...
        fn cleanup(&mut self, gl: Option<&ffi::Gles2>) {
            ...
            if let Some(gl) = gl {
                unsafe {
                    gl.DeleteQueriesEXT(
                        self.pool.len() as ffi::types::GLsizei,
                        self.pool.as_ptr().cast(),
                    );
                }
            }
    ...
    impl Drop for QueryPool {
        fn drop(&mut self) {
            debug_assert!(
                self.pool.is_empty(),
                "QueryPool must be cleaned up before drop"
            );
        }
    }
    ...
    impl GpuProfiler {
        pub fn new(gl: &ffi::Gles2, extensions: &[String]) -> Self {
            let has_timer_query = extensions
                .iter()
                .any(|ext| ext == "GL_EXT_disjoint_timer_query");
            if !has_timer_query {
            ...
            let gpu_name =
                unsafe { CStr::from_ptr(gl.GetString(ffi::RENDERER) as *const c_char).to_str() };
