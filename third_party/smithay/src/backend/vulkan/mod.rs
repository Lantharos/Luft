...
impl Instance {
    ...
    pub unsafe fn with_extensions(
        ...
        let api_version = Version::from_raw(u32::min(
            max_version.to_raw(),
            requested_max_version.to_raw(),
        ));
        ...
        let app_name = app_info
            .map(|info| CString::new(info.name).expect("app name contains null terminator"));
...
impl PhysicalDevice {
    ...
    pub fn primary_node(&self) -> Result<Option<DrmNode>, UnsupportedProperty> {
        ...
        let node = Some(properties_drm)
            ...
            .and_then(|props| {
                DrmNode::from_dev_id(libc::makedev(
                    props.primary_major as _,
                    props.primary_minor as _,
                ))
                .ok()
            });
    ...
    pub fn render_node(&self) -> Result<Option<DrmNode>, UnsupportedProperty> {
        ...
        let node = Some(properties_drm)
            ...
            .and_then(|props| {
                DrmNode::from_dev_id(libc::makedev(
                    props.render_major as _,
                    props.render_minor as _,
                ))
                .ok()
            });
    ...
    pub unsafe fn get_format_properties(
        &self,
        format: vk::Format,
        props: &mut vk::FormatProperties2<'_>,
    ) {
        let instance = self.instance().handle();
...
impl PartialEq for PhysicalDevice {
    ...
    fn eq(&self, other: &Self) -> bool {
        ...
        self.phd == other.phd
            && self.instance().handle().handle() == other.instance().handle().handle()
    }
}
...
unsafe extern "system" fn vulkan_debug_utils_callback(
    ...
    let _ = std::panic::catch_unwind(|| {
        ...
        let _guard = unsafe { (span as *mut tracing::Span).as_ref() }
            .unwrap()
            .enter();
