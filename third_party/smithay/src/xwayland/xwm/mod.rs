...
use x11rb::{
    ...
    protocol::{
        ...
        xproto::{
            ...
            InputFocus, NotifyDetail, PixmapWrapper, PropMode, Property, QueryExtensionReply,
            Screen, StackMode, Visualid, WindowClass,
        },
    },
...
impl X11Wm {
        ...
        D: 'static,
    {
        ...
        let servertime_counter = if sync_supported {
            conn.sync_list_system_counters()?
                .reply_unchecked()?
                .and_then(|reply| {
                    reply.counters.into_iter().find(|counter| {
                        counter.name == b"SERVERTIME" && counter.counter != x11rb::NONE
                    })
                })
...

        {
            ...
            conn.randr_select_input(
                screen.root,
                NotifyMask::OUTPUT_CHANGE | NotifyMask::SCREEN_CHANGE,
            )?;
        }
    ...
    pub fn raise_window<'a, W: X11Relatable + 'a>(
        &mut self,
        window: &'a W,
    ...
    fn update_stacking_order_impl<'a, W: X11Relatable + 'a>(
        ...
        for relatable in order {
            let stacking_ordered_elems: Vec<&X11Surface> = self
                ...
                .collect();
            let pos = stacking_ordered_elems
                .iter()
    ...
    pub fn set_cursor(
        ...
        let pixmap =
            PixmapWrapper::create_pixmap(&*self.conn, 32, self.screen.root, size.w, size.h)?;
    ...
    pub fn set_randr_primary_output(
        &mut self,
        ...
        for output_xid in res.outputs {
            ...
            if name == output.name() {
                ...
                if output_xid != current_primary {
                    let cookie = self
                        .conn
                        .randr_set_output_primary(self.screen.root, output_xid)?;
                    self.sequences_to_ignore
                        .push(Reverse(cookie.sequence_number() as u16));
                }
                return Ok(());
    ...
    match event {
        Event::CreateNotify(n) => {
            ...
            if xwm
                .windows
                .iter()
                .any(|s| s.mapped_window_id() == Some(n.window))
            {
                return Ok(());
            }
                            ...
                            if let Some(values) = prop.value32() {
                                let mime_types = values
                                    .filter_map(|val| match val {
                                        val if val == xwm.atoms.TARGETS
                                            || val == xwm.atoms.TIMESTAMP =>
                                        {
                                            None
                                        }
                            ...
                            .insert_source(
                                ...
                                move |_, fd, data| {
                                    ...
                                    if let Some(transfer) =
                                        selection.incoming.get_mut(&incoming_window)
                                    {
                                        match write_selection_callback(
                                            fd.as_fd(),
                                            conn,
                                            atoms,
                                            transfer,
            ...
            if allow_access {
                match n.target {
                    x if x == xwm.atoms.TARGETS => {
                        ...
                        let targets =
                            [xwm.atoms.TARGETS, xwm.atoms.TIMESTAMP]
                                .iter()
                                .copied()
                                .chain(selection.mime_types.iter().filter_map(|mime| {
                                    atom_from_mime(mime, &conn, &xwm.atoms).ok()?
                                }))
                                .collect::<Vec<u32>>();
            ...
            match msg.type_ {
                x if x == xwm.atoms.WL_SURFACE_ID => {
                    ...
                    if let Some(surface) = xwm.windows.iter_mut().find(|x| {
                        x.window_id() == msg.window || x.mapped_window_id() == Some(msg.window)
                    }) {
                ...
                x if x == xwm.atoms.WL_SURFACE_SERIAL => {
                        ...
                        .cloned()
                    {
                                ...
                                .clone()
                        {
                            ...
                            XWaylandShellHandler::surface_associated(
                ...
                x if x == xwm.atoms._NET_WM_STATE => {
                        ...
                        .cloned()
                    {
                        ...
                        match &data[1..=2] {
                            ...
                            actions if actions.contains(&xwm.atoms._NET_WM_STATE_MODAL) => {
                                match data[0] {
                            ...
                            actions if actions.contains(&xwm.atoms._NET_WM_STATE_STICKY) => {
                                match data[0] {
                            ...
                            actions if actions.contains(&xwm.atoms._NET_WM_STATE_SHADED) => {
                                match data[0] {
                            ...
                            actions
                                if actions.contains(&xwm.atoms._NET_WM_STATE_DEMANDS_ATTENTION) =>
                            {
                                match data[0] {
                ...
                x if x == xwm.atoms._NET_ACTIVE_WINDOW => {
                    ...
                    let currently_active_window = xwm
                        .windows
                        .iter()
                        .find(|x| x.window_id() == data[2])
                        .cloned();
                    if let Some(surface) = xwm
                        .windows
                        .iter()
                    ...
                    match data[0] {
                        x if x == xwm.atoms._NET_WM_PING => {
                            ...
                            let surface = (window_id != x11rb::NONE)
                                ...
                                .flatten()
                                .filter(|surface| {
                                    surface.state.lock().unwrap().pending_ping_timestamp
                                        == Some(timestamp)
                                })
                                .or_else(|| {
                                    xwm.windows.iter().find(|x| {
                                        x.state.lock().unwrap().pending_ping_timestamp
                                            == Some(timestamp)
                                    })
                                })
                                .cloned();
                ...
                x if x == xwm.atoms.XdndStatus => {
                    ...
                    if let Some(surface) = xwm.windows.iter().find(|x| x.window_id() == window_id) {
                        xwm.dnd.handle_status(
                            surface,
                            msg.data,
                            &xwm.atoms,
                            xwm.client_scale.clone(),
                        );
                    }
                }
                ...
                x => {
                    debug!(
                        ...
                        String::from_utf8(conn.get_atom_name(x)?.reply_unchecked()?.unwrap().name)
                            .ok()
                    )
                }
            }
