...
use crate::{
    backend::renderer::{
        ContextId, ErasedContextId, ImportAll, Renderer, Texture, buffer_dimensions,
        buffer_has_alpha, element::RenderElement,
    },
    utils::{
        Buffer as BufferCoord, Coordinate, Logical, Physical, Point, Rectangle, Scale, Size,
        Transform,
    },
    wayland::{
        compositor::{
            self, BufferAssignment, Damage, RectangleKind, SUBSURFACE_ROLE, SubsurfaceCachedState,
            SurfaceAttributes, SurfaceData, TraversalAction, add_destruction_hook,
            is_sync_subsurface, with_surface_tree_downward, with_surface_tree_upward,
        },
        viewporter,
    },
};
    ...
    pub(crate) fn update_buffer(&mut self, states: &SurfaceData) {
        ...
        if new_buffer || surface_view_changed {
            ...
            if !self.buffer_has_alpha.unwrap_or(true) {
                self.opaque_regions
                    .push(Rectangle::from_size(surface_view.dst))
                ...
                let opaque_regions = region_attributes
                    ...
                    .map(|(kind, rect)| {
                        ...
                        let rect_constrained_loc =
                            rect.loc.constrain(Rectangle::from_size(dest_size));
                        ...
                        |mut new_regions, (kind, rect)| {
                            match kind {
                                ...
                                RectangleKind::Subtract => {
                                    new_regions = Rectangle::subtract_rects_many_in_place(
                                        new_regions,
                                        [rect],
                                    );
                                }
