...
use relay_core::{LogoSource, RelayVisualState};
use relay_display::FrameBuffer;

use crate::bmp::{bmp_dimensions, decode_bmp, DecodedBmp};
use crate::tux;

const OEM_PATHS: &[&str] = &[
    "/sys/firmware/acpi/bgrt/image",
    "/usr/share/relay/oem/logo.bmp",
    "/usr/lib/relay/oem/logo.bmp",
];

pub const LINUX_LOGO_NATIVE: (u32, u32) = (80, 96);

pub fn draw_logo(frame: &mut FrameBuffer, state: &RelayVisualState) {
...
fn try_draw_bmp_path(frame: &mut FrameBuffer, state: &RelayVisualState, path: &str) -> bool {
...
fn blit_logo(frame: &mut FrameBuffer, bmp: &DecodedBmp, state: &RelayVisualState) {
...
fn draw_linux_fallback(frame: &mut FrameBuffer, state: &RelayVisualState) {
    let r = state.logo_rect;
