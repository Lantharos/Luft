...
impl OutputDamageTracker {
        ...
        R::TextureId: Texture,
    {
        ...
        let (output_size, output_scale, output_transform) =
            std::convert::TryInto::<(Size<i32, Physical>, Scale<f64>, Transform)>::try_into(
                &self.mode,
            )?;
        ...
        E: Element,
    {
        ...
        let mut element_visible_area_workhouse =
            std::mem::take(&mut self.element_visible_area_workhouse);
        ...
        for element in elements.iter() {
            ...
            let element_output_geometry =
                match element.geometry(output_scale).intersection(output_geo) {
                    Some(geo) => geo,
                    None => continue,
                };
        ...
        let render_res = (|| {
            ...
            for (z_index, element) in render_elements.iter().rev().enumerate() {
                ...
                element_opaque_regions.extend(
                    self.opaque_regions
                        [element_opaque_regions_range.start..element_opaque_regions_range.end]
                        .iter()
                        .copied()
                        .map(|mut rect| {
