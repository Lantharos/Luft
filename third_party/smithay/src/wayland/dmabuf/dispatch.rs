...
use super::{
    DmabufData, DmabufFeedbackData, DmabufGlobal, DmabufGlobalData, DmabufHandler,
    DmabufParamsData, Import, ImportNotifier, Modifier, SurfaceDmabufFeedbackState,
};
        ...
        match request {
            ...
            zwp_linux_dmabuf_v1::Request::GetSurfaceFeedback { id, surface } => {
                ...
                if compositor::with_states(&surface, |states| {
                    states
                        .data_map
                        .get::<SurfaceDmabufFeedbackState>()
                        .is_none()
                }) {
                ...
                let surface_feedback = compositor::with_states(&surface, |states| {
                    let feedback_state =
                        states.data_map.get::<SurfaceDmabufFeedbackState>().unwrap();
...
impl<D> Dispatch2<zwp_linux_dmabuf_feedback_v1::ZwpLinuxDmabufFeedbackV1, D>
    for DmabufFeedbackData
{
    fn request(
        &self,
        _state: &mut D,
    ...
    fn request(
        ...
        match request {
            ...
            zwp_linux_buffer_params_v1::Request::SetSamplingDevice { device } => {
                if let Ok(array) = device.try_into() {
                    *self.node.lock().unwrap() = Some(dev_t::from_ne_bytes(array));
                } else {
                    params.post_error(
                        zwp_linux_buffer_params_v1::Error::InvalidDevTSize,
                        "Could not parse the dev_t provided via SetSamplingDevice",
                    );
                }
            }
            ...
            zwp_linux_buffer_params_v1::Request::Create {
                ...
                if let Some(dmabuf) = self.create_dmabuf(
                    params,
                    width,
                    height,
                    format,
